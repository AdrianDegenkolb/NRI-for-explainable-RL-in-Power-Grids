...
```bash
python experiments/train.py training=ppo model=ragnn env=case14
```
...
| File | Purpose |
|---|---|
| `ragnn.yaml` | **Full model** — Graphormer encoder + Relation-Aware GNN (RAGNN). Learns latent edge probabilities and uses them to condition the GNN message passing. |
| `gnn.yaml` | GNN with fixed (precomputed) edge probabilities — no encoder, no latent graph learning. |
...
| File | Purpose |
|---|---|
| `graph.yaml` | Graph-structured observation: node feature matrix `[B, N, F]`, edge index, edge mask. Required for GNN/RAGNN models. |
...
### `relation_awareness/`
Controls whether the encoder is active and how the KL regularisation is scheduled.

| File | Purpose |
|---|---|
| `default.yaml` | **Enabled** — KL loss active, Gumbel-Softmax temperature annealed τ: 1.5→0.5, separate β weights for graph and latent edges |
| `disabled.yaml` | **Disabled** — KL weights set to 0.0, τ fixed. Use this with `model=gnn` or `model=mlp` baselines. |
...
| File | Purpose |
|---|---|
| `default.yaml` | Enables `AnnealingCallback` (decays β and τ over training) and `TuneCallback` (logs custom metrics for Ray Tune) |
...
| File | Purpose |
|---|---|
| `optuna.yaml` | Run 50 Optuna trials sweeping over encoder and relation-awareness hyperparameters |
