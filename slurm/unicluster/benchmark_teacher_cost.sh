...
# Phase 0: benchmark N-1 Teacher cost-per-interaction on one grid — BWUniCluster.
#
...
# the SLURM array walltime for the full teacher run (slurm/unicluster/teacher_n1_array.sh).
#
# Runs in the separate `curriculum` conda environment (environment_curriculum.yaml) — never L2RPN.
#
# Usage:
#   slurm/unicluster/benchmark_teacher_cost.sh <env_name> <experiment_name> <lines_to_attack...>
#
# Example:
#   slurm/unicluster/benchmark_teacher_cost.sh l2rpn_wcci_2020_train phase0_case36 3 12 27 41 58
...
# 25 interactions comfortably fits BWUniCluster's 30 min dev partitions on the smaller (36-bus)
