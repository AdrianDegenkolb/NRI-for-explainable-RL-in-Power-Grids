...
# N-1 Teacher action-space generation — SLURM array, chunked over chronics — HoreKa.
#
...
# one-chronic-per-task (confirmed on BWUniCluster: sbatch rejected a 2592-wide array outright
# with "Invalid job array specification" — likely the same limit applies here) — so chronics
# are chunked into batches, each batch handled sequentially by one array task, all appending to
# the same shard CSV.
#
# Run slurm/horeka/teacher_n1_smoketest.sh first to validate the pipeline on one chronic —
...
# chronic and biases the resulting action set.
#
# Runs in the separate `curriculum` conda environment (environment_curriculum.yaml) — never L2RPN.
#
# Usage:
#   slurm/horeka/teacher_n1_array.sh <grid> <experiment_name> <max_concurrent> <max_array_size> <lines_to_attack...>
#
# Arguments:
#   grid              case36 | case118
...
#                     500 is a conservative guess — check the real limit with:
...
#
# Example:
#   slurm/horeka/teacher_n1_array.sh case36 2026_09_20_teacher_n1_case36 50 500 3 12 27 41 58 60 71 88 95 101
