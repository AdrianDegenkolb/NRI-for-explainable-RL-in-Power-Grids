#!/bin/bash
# CPU Efficiency Experiment v3 — 4 variants, single seed, IEEE14 RAPPO
# Builds on v2 results. Adds graph sparsification (threshold=0.05) and
# disables RAGNN diagnostics. Longer training (10000 steps). Pretraining enabled.
#
...
#
# Usage: bash experiments/slurm/horeka/cpu_efficiency_v3.sh
experiment_name=$(date +%Y_%m_%d)_IEEE14_cpu_efficiency_v3
...
# ---------------------------------------------------------------------------
BASE_ARGS="training=ppo model=ragnn obs_space=graph relation_awareness=default experiment.seed=0 experiment.nb_timesteps=10000 rollouts.num_rollout_workers=48 experiment.post_training_evaluation.enabled=False model.gnn.sparsify_threshold=0.05 model.gnn.diagnose_every=0"
...
#SBATCH --time=0:30:00
#SBATCH --mem=100G
#SBATCH --partition=accelerated,accelerated-h100,dev_accelerated,dev_accelerated-h100
#SBATCH --account=hk-project-pai00074
